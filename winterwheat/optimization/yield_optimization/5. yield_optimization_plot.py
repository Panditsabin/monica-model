...
import pandas as pd
import os
from monica_run_postprocess import(map_parameters, update_parameter_files, crop_json, run_monica, process_yield_file)
import numpy as np
...
def process_and_plot_condition(ax, color, best_x, param_names, proj_dir, parameter_dir, 
    ...
    update_parameter_files(param_update, parameter_dir, proj_dir, 
                           cultivar_file, species_file, crop_general_file, 
                           sim_file, sim_start, sim_end, i)

    # crop json
    crop_json(irr_path, fert_path, crp_path, sim_start, sim_end, 
              proj_dir, parameter_dir, set_name, i)

    # monica run
    run_monica(proj_dir, set_name, i)
    ...
    df_obs = pd.read_excel(obs_path, sheet_name="WinterWheat")
...
if __name__ == '__main__':
    ...
    sim_start, sim_end = '2007-07-01', '2022-12-31'
