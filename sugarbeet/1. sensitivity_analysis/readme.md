...
| File | Description |
|------|-------------|
| `sugarbeet_morris_multiprocess.py` | Consists — JSON mapping, parameter file updates, MONICA runner, and output parsers |
| `morris_run_sb_sa.py` | Execution script — runs parallel Morris simulations and saves consolidated results |
| `morris_sa_sugarbeet_optimal_condn.ipynb` | Parameter generation, objective function calculation, statistical analysis and visualizations |
...
**Output:** `sugarbeet_morris.xlsx` — input sample file for the simulation runner
 
### Phase 2 — Automated Simulation
 
**Module:** `morris_run_sb_sa.py`
 
Reads trajectories from `sugarbeet_morris.xlsx`, distributes simulations across `n_cores` parallel workers, and for each trajectory updates the MONICA `.json` parameter files, executes `monica-run.exe`, and extracts yield, irrigation and soil moisture outputs.
...
- **Yearly SI Boxplots** — sensitivity fluctuation across growing seasons (2009–2020)
...
| `excluded_years` | [2017] | No-crop years excluded from results |
| `n_cores` | 15 | Parallel workers — adjust to available CPU cores |
 
## Setting Up Paths
 
All paths are configured at the top of `morris_run_sb_sa.py`. Set `base_path` to your local MONICA root directory and all other paths resolve automatically relative to it.
...
| File | Contents |
|------|----------|
| `sugarbeet_yld.txt` | Simulated yearly yield per trajectory |
