...
from monica_run_postprocess_sb import (
...
from post_process_sb import read_optimization_result
...
if __name__ == '__main__':
    ...
    crop_rot_opt = os.path.join(base_path, r'magagement_data\sugarbeet_optimal\crop_mgmt_ZR.xlsx')
    ...
    crop_rot_red = os.path.join(base_path, r'magagement_data\sugarbeet_reduced\crop_mgmt_ZR.xlsx')
    ...
    man_irr_opt = os.path.join(base_path, r"field_obs_data\sugarbeet_irr_opt_condition.xlsx")
    man_irr_red = os.path.join(base_path, r"field_obs_data\sugarbeet_irr_red_condition.xlsx")

    # param
    para_df = pd.read_excel(
        os.path.join(base_path, r'sensitive_paras\sugarbeet_yld_sm_sens_para.xlsx')
    )
    ...
    # sim period
    sim_start, sim_end = '2009-01-01', '2020-12-31'
    
    # parameters and simulation file will be saved inside this folder
    set_name = "pareto_eval"

    parameter_dir = os.path.join(base_path, r'monica-parameters\sugarbeet_3objs', set_name)
    project_dir_opt = os.path.join(base_path, r'projects\sugarbeet_3objs_opt', set_name)
    project_dir_red = os.path.join(base_path, r'projects\sugarbeet_3objs_red', set_name)

    result_path = os.path.join(base_path, r'projects\sugarbeet_3objs_result')
    ...
    # yld
    df_obs_yield_opt = pd.read_excel(obs_yield_opt, sheet_name="Sugarbeet")
    df_obs_yield_opt.columns = ["Year", "Obs"]
    df_obs_yield_opt = df_obs_yield_opt.set_index("Year")

    df_obs_yield_red = pd.read_excel(obs_yield_red, sheet_name="Sugarbeet")
