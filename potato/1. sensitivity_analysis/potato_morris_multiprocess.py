...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov  1 17:40:47 2025
...
def update_parameter_files(base_path, param_update, parameter_dir, project_dir, cultivar_file, species_file,  crop_general_file, sim_file, sim_start, sim_end, run_name):
    ...
    #  directories 
    subfolder_project = os.path.join(project_dir, run_name)
    subfoler_parameter = os.path.join(parameter_dir, run_name)
...
# 1. site json
def site_json(base_path, project_dir, set_name, run_name):
    """
    Creates site.json and saves it in the project folder alongside crop.json.
   
    """
    # path to save the site json
    save_path = os.path.join(project_dir, run_name, set_name)
...
def crop_json(base_path, irr_path, fert_path, crp_path, sim_start, sim_end, 
              project_dir, parameter_dir, set_name, run_name):
    """
    ...
    set_param_path = os.path.join(base_monica, parameter_dir, run_name, set_name)
...
def run_monica(base_path, project_dir, set_name, run_name):
    ...
    try:
        ...
        print(f"Simulation failed for {set_name} in Run {run_name}")
