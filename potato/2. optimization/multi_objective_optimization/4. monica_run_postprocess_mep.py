...
def update_parameter_files(base_path, param_update, parameter_dir, project_dir, cultivar_file, species_file,  crop_general_file, sim_file, sim_start, sim_end, condition=None):
...
def crop_json(base_path, irr_path, fert_path, crp_path, sim_start, sim_end, 
              project_dir, parameter_dir, set_name):
    """
    ...
    set_param_path = os.path.join(base_monica, parameter_dir, set_name)
...
# run monica
def run_monica(base_path, project_dir, set_name):
    ...
    try:
        ...
        print(f"Simulation failed for {set_name}")
...
def extract_irr_data(sim_csv_path):
    ...
    try:
        ...
        return irr_df[['Year', 'Sim_irrig']]  # columns Year, Sim_irrig 
