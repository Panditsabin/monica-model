...
from monica_run_postprocess_mep import (
...
from post_process_mep import read_optimization_result
...
# plotting function
def plot_ensemble_condition(ax, df_sim, df_obs, color, title, split_year=2017): 
...
if __name__ == '__main__':
    ...
    crop_rot_opt = os.path.join(base_path, r'magagement_data\potato_optimal\crop_mgmt_MEP.xlsx')
    ...
    crop_rot_red = os.path.join(base_path, r'magagement_data\potato_reduced\crop_mgmt_MEP.xlsx')
    ...
    man_irr_opt = os.path.join(base_path, r"field_obs_data\potato_irr_opt_condition.xlsx")
    man_irr_red = os.path.join(base_path, r"field_obs_data\potato_irr_red_condition.xlsx")

    # param
    para_df = pd.read_excel(
        os.path.join(base_path, r'sensitive_paras\potato_yld_sm_sens_para.xlsx')
    )
    ...
    parameter_dir = os.path.join(base_path, r'monica-parameters\potato_3objs', set_name)
    project_dir_opt = os.path.join(base_path, r'projects\potato_3objs_opt', set_name)
    project_dir_red = os.path.join(base_path, r'projects\potato_3objs_red', set_name)

    result_path = os.path.join(base_path, r'projects\potato_3objs_result')
    ...
    # yld
    df_obs_yield_opt = pd.read_excel(obs_yield_opt, sheet_name="Potato")
    df_obs_yield_opt.columns = ["Year", "Obs"]
    df_obs_yield_opt = df_obs_yield_opt.set_index("Year")

    df_obs_yield_red = pd.read_excel(obs_yield_red, sheet_name="Potato")
    ...
    plt.suptitle(
        "Pareto Front Solution Analysis\nSubarbeet (Yield & Irrigation)",
