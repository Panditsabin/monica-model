...
def update_parameter_files(base_path, param_update, parameter_dir, project_dir, cultivar_file, species_file,  crop_general_file, sim_file, sim_start, sim_end, i, condition=None):
    ...
    #  directories 
    subfolder_project = os.path.join(project_dir, f'potato_run{i}')
    subfoler_parameter = os.path.join(parameter_dir, f'potato_run{i}')
    ...
    for set_name, updates in param_update.items():
        ...
        for parameter_file, file_path in source_files.items():
            ...
            # 4. Simulation Parameters Update 
                ...
                # Update threshold based on condition (threshold_opt or threshold_red)
                threshold_key = f"threshold_{condition}"
                if threshold_key in updates:
                    data["AutoIrrigationParams"]["trigger_if_nFC_below_%"][0] = int(updates[threshold_key])
             ...
             # 5. Save Files
            file_map = {
                 "SpeciesParameters": "potato.json",
                 "CultivarParameters": "moderately-early-potato.json",
...
def site_json(base_path, project_dir, set_name, i):
    ...
    save_path = os.path.join(project_dir, f'potato_run{i}', set_name)
...
def crop_json(base_path, irr_path, fert_path, crp_path, sim_start, sim_end, 
    ...
    set_param_path = os.path.join(base_monica, parameter_dir, f'potato_run{i}', set_name)
    ...
    crop_json_data = {
        'crops': {
            "MEP": {
                ...
                "cropParams": {
                    "species": ["include-from-file", get_rel("potato.json")],
                    "cultivar": ["include-from-file", get_rel("moderately-early-potato.json")]
                },
...
def run_monica(base_path, project_dir, set_name, i):
    ...
    set_path = os.path.join(project_dir, f"potato_run{i}", set_name)
    sim_json = "sim.json"
...
def simulated_yield_data(sim_csv_path):
    """
    Reads MONICA sim-out.csv
    Extracts max yield per year and converts kgDM/ha to tDM/ha.
    """
    try:
        df = pd.read_csv(sim_csv_path, low_memory=False, skiprows=1, sep=',')
        ...
        return yield_df[['Year', 'Sim_yield']]  # column Year, Sim_yield

    except Exception as e:
        print(f"Error reading simulation file {sim_csv_path}: {e}")
        return pd.DataFrame(columns=['Year', 'Sim_yield'])


# irrigation data 
def extract_irr_data(sim_csv_path):
    """
    Reads MONICA sim-out.csv
    Extracts total annual irrigation sum in mm.
    """
    try:
        df = pd.read_csv(sim_csv_path, low_memory=False, skiprows=1, sep=',')

        df['Year'] = pd.to_numeric(df['Year'], errors='coerce')
        df['Irrig'] = pd.to_numeric(df['Irrig'], errors='coerce')
        df = df.dropna(subset=['Year', 'Irrig'])

        irr_df = df.groupby('Year')['Irrig'].sum().reset_index()
        irr_df.rename(columns={'Irrig': 'Sim_irrig'}, inplace=True)

        return irr_df[['Year', 'Sim_irrig']]  # columns Year, Sim_irrig (mm)

    except Exception as e:
        print(f"Error reading simulation file {sim_csv_path}: {e}")
        return pd.DataFrame(columns=['Year', 'Sim_irrig'])


# extract the simulated moisture data
def extract_moist_data(sim_csv_path):
    df = pd.read_csv(sim_csv_path, sep=',', skiprows=1)  
    
    for col in ['Mois_1', 'Mois_2', 'Mois_3']:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df['avg_moist_30'] = df[['Mois_1', 'Mois_2', 'Mois_3']].mean(axis=1)
    
    moist_data = df[['Date', 'avg_moist_30']].copy()
    moist_data['Date'] = pd.to_datetime(moist_data['Date'])
    
    return moist_data   # col name Date , avg_moist_30


# calculate objectives

def calculate_objectives(set_name, project_dir_opt, project_dir_red,
                         obs_yield_path_opt, obs_yield_path_red,
                         sheet_name, obs_moist_path_opt, obs_moist_path_red,
                         manual_irr_path_opt, manual_irr_path_red, sim_start, sim_end,
                         i):
    """
    Calculates 3 RMSE objectives (yield, soil moisture, irrigation)
    across both Optimal and Reduced irrigation scenarios combined.
    Returns (yield_rmse, moist_rmse, irr_rmse).
    """
    excluded_years = [2016]

    sim_csv_opt = os.path.join(project_dir_opt, f'potato_run{i}', set_name, 'sim-out.csv')
    sim_csv_red = os.path.join(project_dir_red, f'potato_run{i}', set_name, 'sim-out.csv')

    try:
        # yld
        sim_yield_opt = simulated_yield_data(sim_csv_opt)
        sim_yield_red = simulated_yield_data(sim_csv_red)

        obs_yield_opt = pd.read_excel(obs_yield_path_opt, sheet_name)
        obs_yield_red = pd.read_excel(obs_yield_path_red, sheet_name)

        def merge_yield(sim, obs):
            sim = sim[~sim['Year'].isin(excluded_years)]
            return pd.merge(sim, obs, on="Year", how="inner")

        yield_merged = pd.concat([
            merge_yield(sim_yield_opt, obs_yield_opt),
            merge_yield(sim_yield_red, obs_yield_red)
        ], ignore_index=True)

        # sm
        sim_moist_opt = extract_moist_data(sim_csv_opt)
        sim_moist_red = extract_moist_data(sim_csv_red)

        obs_moist_opt = pd.read_excel(obs_moist_path_opt)
        obs_moist_opt['Date'] = pd.to_datetime(obs_moist_opt['Date'])
        obs_moist_red = pd.read_excel(obs_moist_path_red)
        obs_moist_red['Date'] = pd.to_datetime(obs_moist_red['Date'])

        def merge_moist(sim, obs):
            sim = sim[sim['Date'].between(pd.to_datetime(sim_start),
                                          pd.to_datetime(sim_end))]
            return pd.merge(sim, obs, on="Date", how="inner")

        moist_merged = pd.concat([
            merge_moist(sim_moist_opt, obs_moist_opt),
            merge_moist(sim_moist_red, obs_moist_red)
        ], ignore_index=True)

        # irr
        sim_irr_opt = extract_irr_data(sim_csv_opt)
        sim_irr_red = extract_irr_data(sim_csv_red)

        man_irr_opt = pd.read_excel(manual_irr_path_opt)
        man_irr_red = pd.read_excel(manual_irr_path_red)

        def merge_irr(sim, obs):
            sim = sim[~sim['Year'].isin(excluded_years)]
            return pd.merge(sim, obs, on="Year", how="inner")

        irr_merged = pd.concat([
            merge_irr(sim_irr_opt, man_irr_opt),
            merge_irr(sim_irr_red, man_irr_red)
        ], ignore_index=True)

        # rmse
        yield_rmse = np.sqrt(np.mean((yield_merged["Sim_yield"]    - yield_merged["Obs_yield"])**2))
        moist_rmse = np.sqrt(np.mean((moist_merged["avg_moist_30"] - moist_merged["obs moist_30 cm"])**2))
        irr_rmse   = np.sqrt(np.mean((irr_merged["Sim_irrig"]      - irr_merged["manual_irrigation"])**2))

        return yield_rmse, moist_rmse, irr_rmse

    except Exception as e:
        print(f'Error in objective calculation for {set_name}: {e}')
        return 1e6, 1e6, 1e6
